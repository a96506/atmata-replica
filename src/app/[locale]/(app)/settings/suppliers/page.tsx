  ...
  return (
    <DocumentList title="Suppliers" subtitle="Bank + tax info · WHT flag.">
      <DataTable
        ...
        rows={rows.map((s) => [
          <Link key="n" href={`/${locale}/settings/suppliers/${s.id}`} className="font-medium text-primary hover:underline">
            {s.name}
          </Link>,
          s.vatNumber ?? "—",
          ...
          s.paymentTermId,
          s.whtApplicable ? (
            <span key="w" className="rounded-full bg-status-pending-muted px-2 py-0.5 text-xs text-status-pending-foreground">
