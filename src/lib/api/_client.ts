 ...
 * fetch.
 *
 * Backend: InsForge (see `src/lib/insforge/*`, `.env.local`). Flip
 * `USE_FAKE_SERVICE` to `false` and replace `fakeService.*` with
 * `@insforge/sdk` / server client calls. UI function signatures stay stable.
 */
...
/** Flip to `false` when domain modules call InsForge instead of fakeService. */
export const USE_FAKE_SERVICE = true;

/** AI FastAPI proxy removed — keep flag false so callers stay on mocks. */
export const USE_AI_BACKEND = false;
