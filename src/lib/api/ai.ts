/**
 * Atmata — AI co-pilot suggestion engine.
 *
 * When `USE_AI_BACKEND` is true, routes requests to the Next.js API proxy
 * at `/api/ai/suggest` which forwards to the FastAPI AI service.
 * Otherwise, uses the deterministic mock rules over seed data.
 *
 * Queued actions are persisted to the backend via `POST /api/ai/queued`
 * when the backend is available, falling back to sessionStorage.
 */
...
import { USE_AI_BACKEND } from "./_client";
import {
  GOODS_RECEIPTS,
  PURCHASE_ORDERS,
  VENDOR_BILLS,
...
/**
 * Fetch AI suggestions for a given scope.
 *
 * When `USE_AI_BACKEND` is true, calls `/api/ai/suggest` (proxied to the
 * FastAPI AI service). Falls back to the deterministic mock on failure.
 */
export async function getAiSuggestions(
  scope: AiSuggestionScope,
): Promise<AiSuggestion[]> {
  // Try backend first when enabled
  if (USE_AI_BACKEND && scope.kind === "doc") {
    try {
      ...
      // Backend unavailable — fall through to deterministic mock
      // eslint-disable-next-line no-console
      console.info("atmata:event", "ai.backend.fallback", { scope });
    }
  }
...
/* ------------------------------------------------------------------ *
 *  Queued actions (Auto mode) — persistent storage.
 *
 *  When USE_AI_BACKEND is true, queued actions are persisted via the
 *  Next.js API proxy to the FastAPI AI service. Otherwise falls back
 *  to sessionStorage.
 * ------------------------------------------------------------------ */
...
export function recordQueuedAction(rec: Omit<QueuedActionRecord, "id" | "queuedAt">): QueuedActionRecord {
  ...
  // Always persist to sessionStorage as immediate local cache
  ...
  // Also persist to backend when enabled (fire-and-forget)
  if (USE_AI_BACKEND) {
    fetch("/api/ai/queued", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      ...
      // Backend unavailable — sessionStorage is the fallback
    });
  }
 ...
 * List queued actions.
 *
 * When the backend is available, fetches from `/api/ai/queued` and merges
 * with any local sessionStorage entries. Otherwise reads sessionStorage only.
 */
export async function listQueuedActions(): Promise<QueuedActionRecord[]> {
  if (typeof window === "undefined") return [];

  // Always read local cache first for instant rendering
  let local: QueuedActionRecord[] = [];
  try {
    local = JSON.parse(window.sessionStorage.getItem(QUEUED_KEY) ?? "[]");
  } catch {
    local = [];
  }

  // Try backend when enabled
  if (USE_AI_BACKEND) {
    try {
      const response = await fetch("/api/ai/queued", { method: "GET" });
      if (response.ok) {
        const remote: QueuedActionRecord[] = await response.json();
        // Merge: deduplicate by id, prefer remote
        const merged = new Map<string, QueuedActionRecord>();
      ...
      // Backend unavailable — return local only
    }
  }
